...
class YamlClass {
  ...
  // Add mixins to handle Logger classes that have circular references using reflection
  try {
    mapper.addMixIn(Class.forName("ch.qos.logback.classic.Logger"), classOf[LoggerMixin])
  } catch {
    case _: ClassNotFoundException => // ignore
  }

  try {
    mapper.addMixIn(Class.forName("ch.qos.logback.classic.LoggerContext"), classOf[LoggerContextMixin])
  } catch {
    case _: ClassNotFoundException => // ignore
  }
  ...
  try {
    mapper.addMixIn(Class.forName("pro.datawiki.sparkLoader.traits.LoggingTrait"), classOf[LoggingTraitMixin])
  } catch {
    case _: ClassNotFoundException => // ignore
  }
