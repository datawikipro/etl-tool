...
class LoaderMinIoBatch(format: FileBaseFormat, configYaml: YamlConfig, configLocation: String, useHiveMetastore: Boolean = false) extends LoaderMinIo(format, configYaml, configLocation) with FileStorageTrait with LoggingTrait {
  ...
  private def write(df: DataFrame, tableName: String, inFormat: String, inWriteMode: WriteMode, finishLocation: String, useCache: Boolean, partitionColumns: List[String]): Unit = {
    ...
    try {
      ...
      var writer = dataFrameToWrite.write.format(inFormat).mode(inWriteMode.toSparkString)

      writer = inWriteMode match {
        case WriteMode.`overwritePartition` => writer
