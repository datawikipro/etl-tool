...
import pro.datawiki.sparkLoader.connection.fileBased.FileStorageCommon
...
class LoaderLocalBase(configYaml: YamlConfig) {
  ...
  def getLocation(location: String, keyPartitions: List[String], valuePartitions: List[String]): String =
    s"${configYaml.folder}/" + FileStorageCommon.buildPartitionedLocation(location, keyPartitions, valuePartitions)
