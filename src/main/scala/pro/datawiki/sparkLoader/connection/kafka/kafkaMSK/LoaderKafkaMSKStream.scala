...
class LoaderKafkaMSKStream(configYaml: YamlConfig, configLocation: String) extends LoaderKafkaTemplateStream(configYaml.`bootstrap.servers`)  with ConnectionTrait with QueryTrait {
...
object LoaderKafkaMSKStream extends YamlClass {
  def apply(inConfig: String): LoaderKafkaMSKStream = {
    val configYaml: YamlConfig = mapper.readValue(getLines(inConfig), classOf[YamlConfig])
    return new LoaderKafkaMSKStream(configYaml, inConfig)
  }
}