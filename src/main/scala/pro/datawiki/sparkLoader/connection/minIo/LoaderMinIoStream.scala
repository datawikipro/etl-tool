...
import pro.datawiki.sparkLoader.connection.minIo.minioStream.LoaderMinIoStream
...
object LoaderMinIoStream extends YamlClass {
  def apply(inConfig: String, mode: FileStorageType): LoaderMinIo = {
    ...
    val loader = mode match
      ...
      case FileStorageType.json => new LoaderMinIoStream(FileBaseFormat.json, configYaml, inConfig)
