  ...
  override def getCoreDag: List[CoreDag] = {
    ...
    val columns: List[String] = {
      if list.nonEmpty then {
        val a = list.map(row => row.getAs[String]("columns")).head
        if a != null then {
          a.split(",").toList
        } else {
          List.apply()
    ...
    val primary_key_columns: List[String] = {
      if list.nonEmpty then {
        val a = list.map(row => row.getAs[String]("primary_key_columns")).head
        if a != null then {
          a.split(",").toList
        } else {
          List.apply()
      ...
      transform = List.apply(
        YamlDataTemplateTransformation(
          ...
          extractAndValidateDataFrame = YamlDataTemplateTransformationExtractAndValidateDataFrame(
            dataFrameIn = "src",
            configLocation = s"/opt/etl-tool/config/schemas/postgresdb/$sourceSchema/$sourceTable.yaml"
          ),
          adHoc = null
        ),
      ),
