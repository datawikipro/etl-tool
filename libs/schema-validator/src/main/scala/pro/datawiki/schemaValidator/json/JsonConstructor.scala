...
class JsonConstructor extends Migration {
  ...
  override def extractStruct(inString: String): BaseSchemaStruct = {
    try {
      ...
      json match
        ...
        case JNull => null
        case _ => {
          throw new SchemaValidationException(s"Неподдерживаемый тип JSON: ${json.getClass.getSimpleName}")
        }
    } catch {
      case e: Exception => {
        throw e
  ...
  override def evolutionSchemaByObject(inJsonString: String, inSchema: BaseSchemaTemplate): BaseSchemaTemplate = {
    try {
      if (inJsonString == null || inJsonString.isEmpty) {
        throw new Exception("Предупреждение: пустая или null JSON строка пропущена")
      }
      val dataInJsonSchemaFormat: BaseSchemaTemplate = extractSchema(inJsonString)

      if (inSchema == null) return dataInJsonSchemaFormat
  ...
  override def validateSchemaForObject(inJsonString: String, inSchema: BaseSchemaTemplate = throw new IllegalArgumentException("Schema is required")): Boolean = {
    try {
      if (inJsonString == null || inJsonString.isEmpty) {
        throw new Exception("Предупреждение: пустая или null JSON строка пропущена")
      }
  ...
  override def writeSchema(schema: BaseSchemaStruct): String = throw new NotImplementedException("Method not implemented")
  ...
  override def getDataFrameFromString(inString: String,
    ...
    if (!inSchema.leftValidate(a.getTemplate)) {
      if (!isStrongValidation) {
        return null
      }
      throw new SchemaValidationException(s"Schema validation failed for JSON data. JSON string: ${inString.take(1000)}${if (inString.length > 1000) "..." else ""}")
    }

    a match {
      case x: BaseSchemaObject => {
        try {
          return BaseSchemaArray(List.apply(x), inSchema, false).packageDataFrame
        } catch {
          case e: Exception => {
            throw e
          }
      ...
      case _ => {
        throw new SchemaValidationException("Only BaseSchemaObject is supported for DataFrame creation")
      }
    }
  }
}