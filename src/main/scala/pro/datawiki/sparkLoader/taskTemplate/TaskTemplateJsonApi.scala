...
class TaskTemplateJsonApi(
  ...
  private def getDataFrameFromJsonNonSchema(json: String): DataFrameTrait = {
    try {
      ...
      val rawDf = migrationJson.getDataFrameFromString(
        ...
        extractedSchema,
        true,
        isStrongValidation = true)(spark)
      return DataFrameOriginal(rawDf)
    } catch {
      case e: Exception => {
  ...
  private def getDataFrameFromTxt(csv: String): DataFrameTrait = {
    ...
    val df = spark.read.
      option("header", "false") // Первая строка - это заголовок
  ...
  private def getResult(variablesList: Map[String, String]): DataFrameTrait = {
    ...
    val df: DataFrameTrait = source.in.resultType match {
      ...
      case "txt" => getDataFrameFromTxt(resTxt)
      case _ => {
