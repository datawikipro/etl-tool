...
import pro.datawiki.sparkLoader.connection.{ConnectionTrait, DatabaseTrait, KafkaMetadataTrait, SupportIdMap}
import pro.datawiki.sparkLoader.context.ApplicationContext
import pro.datawiki.sparkLoader.dictionaryEnum.SCDType.{SCD_0, SCD_2, SCD_3}
...
class LoaderPostgres(configYaml: YamlConfig, configLocation: String) extends ConnectionTrait, DatabaseTrait, KafkaMetadataTrait, SupportIdMap, LoggingTrait {
  ...
  override def writeDfMergeDelta(df: DataFrame, tableSchema: String, tableName: String, scdType: SCDType, columns: List[String], uniqueKey: List[String], partitionBy: List[(String, String)]): Unit = {
    ...
    try {
      ...
      val updateValidIntervalSql: String = scdType match {
        case SCD_3 => {
          s"""
        ...
        case SCD_0 => {
          s"""
             |delete from ${table.getTableAlias} tgt
             |  using ${cache.getLocation}_3 src
             | where src.update_command = 'Update'
             |   and $joinString
             |""".stripMargin
        }
        case _ => {
  ...
  override def runSQL(sql: String): Boolean = {
    ...
    try {
      ...
      case e: Exception => {
        logError("Failed to execute SQL", e, s"sql: $sql") //TODO
        throw e
      }
    }
  }
  ...
  override def readKafkaOffsets(metadataSchema: String, topic: String): Option[String] = {
      val sql =
        s"""
          select '{'|| string_agg( '"'||partition||'": '||"offset",', ' order by partition) ||'}' as result_json
          from ${metadataSchema}.kafka_offset
         where topic = '$topic'
      """
      val offsetDf = getDataFrameBySQL(sql)

      if offsetDf.isEmpty then {
        logInfo(s"No Kafka offsets found for topic: $topic")
        return None
      }

      offsetDf.collect().head.get(0) match {
        case x: String => return Some(x)
        case _ => throw Exception()
      }
      
  }

  override def updateKafkaOffsets(metadataSchema: String,  modify: Map[(String, Int), Long]): Unit = {
    modify.foreach { case (partition, offset) =>
        val upsertSql =
          s"""
          insert into ${metadataSchema}.kafka_offset  (topic, partition, offset, updated_at)
          VALUES ('${partition._1}', ${partition._2}, $offset, NOW())
          ON CONFLICT (topic, partition) 
          DO UPDATE SET offset = EXCLUDED.offset, updated_at = EXCLUDED.updated_at
        """
        runSQL(upsertSql)
        logInfo(s"Updated Kafka offset for topic=${partition._1}, partition=${partition._2} to offset=$offset")
      }
  }
}
...
object LoaderPostgres extends YamlClass {
  ...
  def decodeDataType(in: String): TableMetadataType = {
    in match {
      ...
      case "character" =>  return TableMetadataType.String
      case _ => {
