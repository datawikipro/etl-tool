...
class LoaderMail(configYaml: YamlConfigMail) extends ConnectionTrait {
  ...
  def getDataFrame(email: String,
    ...
    if senderEmail != null then { array = array.appended(new FromTerm(new InternetAddress(senderEmail))) }
    if targetSubject != null then { array = array.appended(new SubjectTerm(targetSubject)) }
    ...
    if messages.isEmpty then { return throw TableNotExistException() }
    ...
    if filteredMessages.isEmpty then { return throw TableNotExistException() }
