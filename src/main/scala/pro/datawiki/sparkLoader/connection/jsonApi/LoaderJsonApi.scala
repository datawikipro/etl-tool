...
case class LoaderJsonApi(in: YamlConfig, configLocation: String) extends ConnectionTrait with LoggingTrait {
  ...
  def getValueUrl(in: String, row: Map[String, String]): String = {
    return WorkWithText.replaceWithoutDecode(in, row, "urlEncode")
  }
  ...
  def retrySend(request2: Request[Either[String, String]], retryNumber: Int, url: String): String = {
    ...
    try {
      ...
      response.body match {
        case Left(e) => {
          logError(s"Request failed: $request2")
  ...
  def sendRequest(variables: Map[String, String]): String = {
    ...
    if in.authType != null then {
      if in.authType.BearerToken != null then {
        request = request.auth.bearer(WorkWithText.replaceWithoutDecode(in.authType.BearerToken, variables))
      }
    }
    ...
    in.method match {
      case "Get" => {
        ...
        val request2: Request[Either[String, String]] = request.get(uri = uri"${WorkWithText.replaceWithoutDecode(in.url, variables)}")
        return retrySend(request2, 0, getValueUrl(in.url, variables))
      }
      case "Post" => {
        if in.formData.nonEmpty then {
          val formMap = in.formData.map(i => i.key -> WorkWithText.replaceWithoutDecode(i.value, variables)).toMap
          request = request.body(formMap)
        }

        if in.body != "" then {
          request = request.contentType("application/json")
          request = request.body(WorkWithText.replaceWithoutDecode(in.body, variables)
          )
        }
