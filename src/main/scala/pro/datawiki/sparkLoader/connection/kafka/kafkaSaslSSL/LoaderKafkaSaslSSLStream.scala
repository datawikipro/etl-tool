...
class LoaderKafkaSaslSSLStream(configYaml: YamlConfig, configLocation: String) extends LoaderKafkaTemplateStream(configYaml.`bootstrap.servers`) with ConnectionTrait with QueryTrait {
...
object LoaderKafkaSaslSSLStream extends YamlClass {
  def apply(inConfig: String): LoaderKafkaSaslSSLStream = {
    val configYaml: YamlConfig = mapper.readValue(getLines(inConfig), classOf[YamlConfig])
    return new LoaderKafkaSaslSSLStream(configYaml, inConfig)
  }
}
