...
class LoaderKafkaMSK(configYaml: YamlConfig, configLocation: String) extends kafkaBase.LoaderKafka(kafkaBase.YamlConfig(`bootstrap.servers` = configYaml.`bootstrap.servers`), configLocation), ConnectionTrait, QueryTrait {
  ...
  override def getCacheKey(): String = {
    ConnectionTrait.generateCacheKey("kafkaAmazon", configLocation)
  }
}
